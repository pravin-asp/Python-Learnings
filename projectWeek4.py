...
def filter_by_year(statistics, year, yearid):
    ...
    """
    statistics_data = list(filter(lambda row : row[yearid] == str(year), statistics))
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    for player in unique_id_list:
        ...
        local_dict = {}
        for field in fields:
            local_dict['player'] = player
            sum_field = 0
            for id_row in id_stat:
                sum_field += float(id_row[field])
            local_dict[field] = int(sum_field)
        agg_stat_dict[player] = local_dict
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    """
    agg_stat_list = []
    for local_dict in aggregate_by_player_id(read_csv_as_list_dict(info["battingfile"], ',', '"'),
                                            info["playerid"],
